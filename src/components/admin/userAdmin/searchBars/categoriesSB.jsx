...
export default function CategoriesSB({categories,name,category,setName,setCategory}){
    ...
    function submitCategorie(e){
        e.preventDefault()
        var result = categories.filter((category)=>category.name === name.toLowerCase())
        setCategory(result?.length ? result : "No se ha encontrado la categoria")
    }
...
<span>Editar categoria</span>
                <form onSubmit={submitCategorie}>
                    <input type="search" value={name} onChange={changeCategorie} placeholder="Search by name..."/>
                    <button type='submit' onClick={submitCategorie}>Buscar</button>
                </form>
        ...
        <div>
            ...
            {category && typeof(category) === "string" ? <p>Categoria no encontrada</p>:<></>}
        </div>
</>)
}